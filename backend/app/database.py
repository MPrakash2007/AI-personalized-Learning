...
def init_db(force: bool = False) -> bool:
    ...
    try:
        ...
        # Safe, non-destructive schema validation for existing tables (Postgres & SQLite)
        # Avoids executing redundant ALTER TABLE or DDL that causes InsufficientPrivilege in serverless cold starts
        try:
                ...
                if "postgres" in dialect:
                    ...
                    if not has_subject:
                        try:
                            ...
                            conn.rollback()
                            logger.info(f"Optional subject column addition skipped: {type(e).__name__}")
            ...
            logger.info(f"Schema upgrade check notice: {clean_mig}")
